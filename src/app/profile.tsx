...
function Row({ label, value, last, danger }: { label: string; value?: string; last?: boolean; danger?: boolean }) {
  return (
    <View className={`py-[14px] px-[16px] flex-row justify-between items-center ${!last ? 'border-b border-b-[#F0EDE4]' : ''}`}>
      ...
      <ScrollView contentContainerClassName="px-[24px] pb-[8px]" showsVerticalScrollIndicator={false}>
        <View className="flex-row items-center gap-[14px] mb-[20px]">
          <View className="w-[58px] h-[58px] rounded-[29px] bg-green items-center justify-center">
            <Text className="font-headline text-[22px] text-white">A</Text>
          </View>
          <View>
            <Text className="font-headline text-[19px] text-text">Alex</Text>
            <Text className="font-body text-[12.5px] text-text-faint">alex@example.com</Text>
          </View>
        </View>
